...
func NewNoiseNetWork(id uint32, addr string, peers map[uint32]common.Peer, msgChan chan *common.Message,
	disPatchChan chan *common.Message, logger logger.Logger, change bool, multi uint32, byzantine bool, f uint32) *NoiseNetWork {
...
func (n *NoiseNetWork) BroadcastMessage(msg *common.Message) {
	...
	for id, p := range n.peers {
		...
		if n.byzantine && (msg.Type == common.Message_PAYLOAD || msg.Type == common.Message_VAL) {
			if id <= 2*n.f+1 {
				continue
			}
		}
...
func (n *NoiseNetWork) SendMessage(id uint32, msg *common.Message) {
	if n.byzantine && msg.Type == common.Message_BVAL {
		return
	}
	m := NoiseMessage{Msg: msg}
...
func (n *NoiseNetWork) OnReceiveMessage(msg *common.Message) {
	if msg.Type == common.Message_SKIP {
		tmp := &common.Message{
	...
	if msg.Type == common.Message_VAL || msg.Type == common.Message_PAYLOAD ||
		msg.Type == common.Message_PASS || msg.Type == common.Message_SKIP {
		n.msgChan <- msg
	} else {
		n.dispatchChan <- msg
	}
