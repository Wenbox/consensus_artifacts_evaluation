...
func (cl *Client) NodeFinish(msg *common.NodeBack, resp *common.Response) error {
	if msg.NodeID == 0 {
		...
		if cl.testMode == "normal" {
			...
			cl.allTime += ((nowTime - cl.startTime[len(cl.clientLatencies)]) * uint64(cl.interval))
			thisLatency += ((nowTime - cl.startTime[len(cl.clientLatencies)]) * uint64(cl.interval))
			cl.consensusLatencies = append(cl.consensusLatencies, msg.SupermaTime)
			cl.clientLatencies = append(cl.clientLatencies, thisLatency)
		}
	} else {
